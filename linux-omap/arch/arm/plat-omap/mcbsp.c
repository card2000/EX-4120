 ...
 * 
 */
void omap_mcbsp_start_srg(unsigned int id, const struct omap_mcbsp_reg_cfg *config)
{
	struct omap_mcbsp *mcbsp;

	mcbsp = id_to_mcbsp_ptr(id);

	 omap2_mcbsp_set_clks_src(id, MCBSP_CLKS_PAD_SRC);
	/* We write the given config */
	__raw_writel(config->spcr2, mcbsp->io_base + OMAP_MCBSP_REG_SPCR2);
	__raw_writel(config->srgr2, mcbsp->io_base + OMAP_MCBSP_REG_SRGR2);
	__raw_writel(config->srgr1, mcbsp->io_base + OMAP_MCBSP_REG_SRGR1);
	__raw_writel(config->pcr0, mcbsp->io_base + OMAP_MCBSP_REG_PCR0);
}
EXPORT_SYMBOL(omap_mcbsp_start_srg);
