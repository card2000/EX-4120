 ...
 * revised for UART #2 wakeup support - Keiichi Matsuhashi 
 * 
	...
	list_for_each_entry(uart, &uart_list, node) {
		if (num == uart->num && uart->can_sleep) {
			if (device_may_wakeup(&uart->pdev->dev))
			{
				...
				printk(KERN_INFO "KENWOOD: [KERNEL-INFO] UART #%d WAKEUP enable!", num);
			}
			else
			{
				omap_uart_disable_wakeup(uart);
//				printk(KERN_INFO "KENWOOD: [KERNEL-INFO] UART #%d WAKEUP disable!", num);
			}
