...
unsigned int mmc_read_data(unsigned int *output_buf, unsigned int blkcnt)
{
	...
	while (blkcnt > 0) {
		...
		if ((mmc_stat & ERRI_MASK) != 0) {
			printf("mmc_read_data: ERRI_MASK\n");
			return mmc_stat;
		}
...
unsigned int omap_mmc_read_sect(mmc_card_data *mmc_c, unsigned int *output_buf,
					unsigned int start_sec, unsigned int blkcnt)
{
	...
	if(blkcnt > 1) {
		OMAP_HSMMC_BLK = MMCSD_SECTOR_SIZE | (blkcnt << 16);
	...
	count = mmc_read_data(output_buf, blkcnt);
	if (count != blkcnt) {
		printf("Read Error: mmc_read_data(blkcnt=%d, count=%d)\n", blkcnt, count);
		return -1;
	}
...
*/
	argument = mmc_card_cur->RCA << 16;
	ret_val = mmc_send_cmd(MMC_CMD13, argument, resp);
	if (ret_val != 1)
		return ret_val;
	...
	argument = mmc_card_cur->RCA << 16;
	ret_val = mmc_send_cmd(MMC_CMD55, argument, resp);
	if (ret_val != 1)
		return ret_val;

	argument = 2;
	ret_val = mmc_send_cmd(MMC_ACMD6, argument, resp);
	if (ret_val != 1)
		return ret_val;
	OMAP_HSMMC_HCTL |= DTW_4_BITMODE;
...
unsigned long mmc_bread(int dev_num, ulong blknr, ulong blkcnt, ulong *dst)
{
	...
	do {
		...
		ret = omap_mmc_read_sect(&cur_card_data, (unsigned int *)dst, (unsigned int)blknr, cur);
		if(ret != cur)
			return ret;
		blocks_todo -= cur;
		blknr += cur;
