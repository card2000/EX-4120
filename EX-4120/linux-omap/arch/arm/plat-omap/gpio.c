 ...
 * Written by Juha Yrjola <juha.yrjola@nokia.com>
 *
 * Copyright (C) 2009 Texas Instruments
 * Added OMAP4 support - Santosh Shilimkar <santosh.shilimkar@ti.com>
 *
 * Copyright (C) 2012 JVC kenwood corporation
 * revised for GPIO #10 wakeup support - Keiichi Matsuhashi 
 * 
...
static int _set_gpio_wakeup(struct gpio_bank *bank, int gpio, int enable)
{
	...
	case METHOD_GPIO_44XX:
		if (bank->non_wakeup_gpios & (1 << gpio)) {
			...
			printk(KERN_INFO "KENWOOD: [KERNEL-INFO] setup GPIO Wakeup #%d\n", gpio);
		}
		spin_lock_irqsave(&bank->lock, flags);
		if (enable)
			bank->suspend_wakeup |= (1 << gpio);
...
static int omap_gpio_suspend(struct sys_device *dev, pm_message_t mesg)
{
	int i;
	unsigned int data;
	struct gpio_bank *bank0 = &gpio_bank[0];
	unsigned long another_flags;

	if (!cpu_class_is_omap2() && !cpu_is_omap16xx())
		return 0;

	/* GPIO-10 for wakeup *//* by Matsuhashi */
	gpio_wake_enable(170, 1);
	spin_lock_irqsave(&bank0->lock, another_flags);
	data = __raw_readl(gpio_bank[0].base + OMAP24XX_GPIO_RISINGDETECT);
	data |= (1 << get_gpio_index(10));
	__raw_writel(data, gpio_bank[0].base + OMAP24XX_GPIO_RISINGDETECT);
	spin_unlock_irqrestore(&bank0->lock, another_flags);
...
/* アプリでエッジトリガーとサスペンドを併用するときはコメントアウトする */
	/* GPIO-10 for wakeup *//* by Matsuhashi */
//	spin_lock_irqsave(&bank0->lock, another_flags);
//	data = __raw_readl(gpio_bank[0].base + OMAP24XX_GPIO_RISINGDETECT);
//	data &= ~(1 << get_gpio_index(10));
