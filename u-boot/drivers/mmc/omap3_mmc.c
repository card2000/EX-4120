...
/*	writel(BLEN_512BYTESLEN | NBLK_STPCNT, &mmc_base->blk);*/
	...
	writel(cmd | CMD_TYPE_NORMAL | CICE_NOCHECK | CCCE_NOCHECK |
		...
		&mmc_base->cmd);
/*	printf("mmc_send_cmd: cmd=0x%08X, arg=0x%08X\n", cmd | CMD_TYPE_NORMAL | CICE_NOCHECK | CCCE_NOCHECK |
		MSBS_SGLEBLK | ACEN_DISABLE | BCE_DISABLE | DE_DISABLE, arg);
*/
	while (1) {
		do {
...
unsigned int mmc_read_data(unsigned int *output_buf, unsigned int blkcnt)
{
	unsigned int mmc_stat;
	...
	while (blkcnt) {
		...
		if ((mmc_stat & ERRI_MASK) != 0) {
			printf("mmc_read_data: ERRI_MASK(stat = 0x%08X)\n", mmc_stat);
			return (unsigned char) mmc_stat;
		}
				...
				mmc_csd_reg_t *cur_csd)
{
	...
	if (mmc_dev_data->mode == SECTOR_MODE) {
		if (mmc_dev_data->card_type == SD_CARD) {
			...
			if (err != 1)
				return err;
			count = mmc_read_data((unsigned int *) &ext_csd, 1);
			if (count != 1)
				return count;
...
unsigned int omap_mmc_read_sect(mmc_card_data *mmc_c, unsigned long *output_buf,
					unsigned int start_sec, unsigned int blkcnt)
{
	unsigned char err;
	...
	if(blkcnt > 1) {
		writel(MMCSD_SECTOR_SIZE | (blkcnt << 16), &mmc_base->blk);
		err = mmc_send_cmd(MMC_CMD18 | MSBS_MULTIBLK | BCE_ENABLE, argument, resp);
		if (err != 1)
			return err;
	} else {
		writel(BLEN_512BYTESLEN | NBLK_STPCNT, &mmc_base->blk);
	...
	count = mmc_read_data((unsigned int *) output_buf, blkcnt);
	if (count != blkcnt) {
		printf("omap_mmc_read_sect: Read error(blkcnt=%d, count=%d)\n", blkcnt, count);
		return count;
	}

	if (blkcnt > 1) {
		argument = 0;
		err = mmc_send_cmd(MMC_CMD12, argument, resp);
		if (err != 1) {
		...
		while(1) {
			err = mmc_send_cmd(MMC_CMD13, argument, resp);
			if (err != 1) {
				printf("omap_mmc_read_sect: MMC_CMD13\n");
				return err;
			} else if(resp[0] & MMC_STATUS_RDY_FOR_DATA) {
				break;
			}
			else {
				;
...
unsigned char configure_mmc(mmc_card_data *mmc_card_cur)
{
	...
	argument = mmc_card_cur->RCA << 16;
	ret_val = mmc_send_cmd(MMC_CMD13, argument, mmc_resp.resp);
	if (ret_val != 1)
		return ret_val;
	...
	/* Configure the bus width to 4 bits */
/*	argument = mmc_card_cur->RCA << 16;
	ret_val = mmc_send_cmd(MMC_CMD55, argument, mmc_resp.resp);
	if (ret_val != 1)
		return ret_val;

	argument = 0;
	ret_val = mmc_send_cmd(MMC_ACMD51, argument, mmc_resp.resp);
	if (ret_val != 1){
		printf("configure_mmc: MMC_ACMD51: ret_val = 0x%08X\n", ret_val);
		return ret_val;
	}

	argument = 0x00FFFFF1;
	ret_val = mmc_send_cmd(MMC_CMD6, argument, mmc_resp.resp);
	if (ret_val != 1)
		return ret_val;
...
*/
	...
	argument = 2;
	ret_val = mmc_send_cmd(MMC_ACMD6, argument, mmc_resp.resp);
	if (ret_val != 1)
		return ret_val;
...
unsigned long mmc_bread(int dev_num, unsigned long blknr, lbaint_t blkcnt,
			void *dst)
{
	unsigned int cur, blocks_todo = (unsigned int)blkcnt;

/*	printf("mmc_bread: blkcnt=%d\n", blkcnt);*/

	do {
		cur = (blocks_todo > CONFIG_SYS_MMC_MAX_BLK_COUNT) ?  CONFIG_SYS_MMC_MAX_BLK_COUNT : blocks_todo;
		if(omap_mmc_read_sect(&cur_card_data, dst, blknr, cur) != cur)
			return 0;

		blocks_todo -= cur;
		blknr += cur;
		dst += cur * MMCSD_SECTOR_SIZE;
	} while (blocks_todo > 0);
